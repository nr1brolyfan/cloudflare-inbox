...
export const appRecoveryCodeRotationReceipt = sqliteTable(
  "app_recovery_code_rotation_receipt",
  {
    operationId: text("operation_id").primaryKey(),
    userId: text("user_id").notNull(),
    expectedPreviousSetId: text("expected_previous_set_id"),
    resultingSetId: text("resulting_set_id").notNull(),
  ...
  (t) => [
    check(
      "app_recovery_code_rotation_receipt_operation_id_check",
      sql`length(operation_id) = 36
        and operation_id = lower(trim(operation_id))
        and substr(operation_id, 9, 1) = '-'
        and substr(operation_id, 14, 1) = '-'
    ...
    check(
      "app_recovery_code_rotation_receipt_user_id_check",
      sql`length(user_id) between 1 and 128 and user_id = trim(user_id)`
    ),
    check(
      "app_recovery_code_rotation_receipt_previous_set_id_check",
      sql`expected_previous_set_id is null
    ...
    check(
      "app_recovery_code_rotation_receipt_resulting_set_id_check",
      sql`length(resulting_set_id) = 36
    ...
    check(
      "app_recovery_code_rotation_receipt_result_check",
      sql`generated_at >= 0
        and committed_at >= generated_at
        and code_count = 10
        and schema_version = 1`
    ),
    index("app_recovery_code_rotation_receipt_user_operation_idx").on(
      t.userId,
      t.operationId
    ),
    uniqueIndex("app_recovery_code_rotation_receipt_resulting_set_idx").on(
      t.resultingSetId
    ),
  ]
);
