...
import { PasskeyCredentialAdministration } from "../auth/passkey-credential-administration";
import type { PasskeyCredentialAdministrationError } from "../auth/passkey-credential-administration";
...
export const PasskeyCredentialManagementGroupLive = HttpApiBuilder.group(
  BackendHttpApi,
      ...
      return handlers
        .handle("list", ({ payload }) =>
          administration
            .list(payload)
            .pipe(
              Effect.catchTag("PasskeyCredentialAdministrationError", mapError)
            )
        )
