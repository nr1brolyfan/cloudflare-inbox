...
- [ ] SAFE-002 Dodać passkey oraz recovery codes albo zatwierdzony równoważny mechanizm odzyskiwania. Guarded passkey enrollment jest gotowy: start i finish wymagają unrestricted session, pięciominutowego step-up oraz tej samej verified external recovery identity; exact RP/origin, discoverable credential i user verification są wymagane, a challenge consumption, credential insert i metadata-only auth audit są atomowe. Passkey sign-in, passkey step-up, credential management oraz recovery codes pozostają otwarte.
