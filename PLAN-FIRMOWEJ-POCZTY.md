...
- [ ] SAFE-006 Dodać operation ID, expected version i readback dla wszystkich nowych mutacji control plane. Mailbox bootstrap/rename, enrollment/verification external recovery identity oraz uwierzytelnione generowanie/rotacja recovery codes mają immutable typed receipts, actor-scoped authenticated readback, exact replay, operation-conflict i unknown-commit recovery. Rotacja kodów zachowuje caller-supplied operation ID przez step-up/network retry, fence'uje dokładny poprzedni aktywny set lub jego brak, atomowo zapisuje 10 hashy, receipt i metadata-only auth audit, a plaintext zwraca wyłącznie przy pierwszym jednoznacznym sukcesie; replay/readback zwraca tylko bezpieczny receipt, również po utraconej odpowiedzi po commit. Publiczne zakończenie account recovery ma osobny caller-generated operation ID i 256-bitowy readback secret przechowywany tylko w przeglądarce; guarded batch atomowo konsumuje dokładny flow i recovery code, tworzy restricted session, metadata-only audit oraz immutable proof-bound receipt. Exact replay i publiczny POST readback zwracają wyłącznie privacy-safe receipt bez cookie lub bearer material, a utraconego jednorazowego cookie nie da się odtworzyć. Restricted sessions nie mogą replayować ani czytać uwierzytelnionych receipt. Zadanie pozostaje otwarte dla pozostałych publicznych mutujących kroków recovery i innych nowych mutacji control plane, aby kontrakt był jednolity i kompletny.
