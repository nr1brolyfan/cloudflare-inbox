...
- [ ] SAFE-006 Dodać operation ID, expected version i readback dla wszystkich nowych mutacji control plane. Mailbox bootstrap/rename oraz enrollment/verification external recovery identity mają już immutable typed receipts, actor-scoped authenticated readback, exact replay, operation-conflict i unknown-commit recovery; recovery identity zapisuje mutation, receipt i metadata-only admin audit atomowo, a restricted sessions nie mogą replayować ani czytać receipt. Zadanie pozostaje otwarte dla pozostałych mutujących kroków recovery i innych nowych mutacji control plane, aby kontrakt był jednolity i kompletny.
