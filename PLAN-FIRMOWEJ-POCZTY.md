...
- [ ] SAFE-002 Dodać passkey oraz recovery codes albo zatwierdzony równoważny mechanizm odzyskiwania. Guarded passkey enrollment jest gotowy: start i finish wymagają unrestricted session, pięciominutowego step-up oraz tej samej verified external recovery identity; exact RP/origin, discoverable credential i user verification są wymagane, a challenge consumption, credential insert i metadata-only auth audit są atomowe. Privacy-safe credential list i revoke są gotowe: API ujawnia wyłącznie opaque record ID oraz czasy utworzenia i ostatniego użycia, chroni ostatni aktywny passkey, ponownie sprawdza token-bound session, step-up i verified recovery w atomowym batchu, zapisuje immutable operation receipt i metadata-only audit oraz obsługuje exact replay i unknown-commit readback. Discoverable passkey sign-in i passkey step-up są gotowe: publiczny start nie przyjmuje user ID ani metadata, oba flow wymagają exact RP/origin i UV, sign-in sprawdza aktywnego użytkownika oraz verified recovery przed przejściem przez AuthFlow, a step-up wiąże challenge z session ID, hashem bieżącej generacji tokenu i wersją polityki, po czym atomowo rotuje sesję przez assureAndRotate. Guarded recovery-code issuance jest gotowe: unrestricted session, non-recovery five-minute step-up, aktywny użytkownik i verified external recovery są ponownie sprawdzane w jednym D1 batchu, który unieważnia poprzedni zestaw, zapisuje 10 hashy i metadata-only audit; plaintext jest zwracany tylko raz i nie jest cache'owany. Publiczny two-proof consume oraz restricted passkey remediation pozostają otwarte.
