...
Dokładne blokady produktu w bieżącym baseline:
...
- Live ingress odrzuca przed routingiem i R2 wiadomości, których zwalidowany Cloudflare `rawSize` przekracza wspólny limit 10 MiB; defense in depth powtarza guard przed ID/czasem i store. Stream nie jest pre-read: `FixedLengthStream` nadal weryfikuje declared-vs-actual length podczas uploadu R2. Nie ma jeszcze stabilnego address/assignment/revision snapshotu ani fencing dla zmiany route.
...
- [x] SAFE-002 Dodać passkey oraz recovery codes albo zatwierdzony równoważny mechanizm odzyskiwania. Guarded passkey enrollment, privacy-safe credential list/revoke, discoverable UV passkey sign-in, token-bound passkey step-up i atomiczne wydawanie 10 hash-only recovery codes są gotowe. Public recovery wymaga dwóch niezależnych dowodów: linku wysłanego na verified external recovery identity oraz jednorazowego recovery code. Consume atomowo sprawdza exact verification row, identity ID/version, aktywnego użytkownika i kod, po czym tworzy piętnastominutową restricted session z exact capability list `second-passkey`; zwykłe application routes oraz core session management odrzucają tę sesję. Recovery passkey ceremony wiąże challenge z session ID, hashem generacji tokenu, exact recovery identity i caller-owned operation contract, wymaga exact RP/origin oraz UV, a końcowy D1 batch dodaje passkey i unrestricted session, zastępuje primary login authority passkey-only identity, unieważnia poprzednie sesje, passkeys, password/TOTP credentials i recovery codes, zapisuje 10 nowych hashy, metadata-only audyty oraz immutable receipt. Plaintext nowych kodów jest zwracany i pokazywany tylko raz przez odpowiedzi `no-store`; replay i proof readback są receipt-only. Produkcyjna dostawa linku działa przez detached `waitUntil`, a publiczny start ma osobny exact rate-limit bucket, 500 ms floor i enumeration-safe odpowiedź. Bezpośrednie testy obejmują eligible/ineligible start, delivery failure, dependency failure, exact proof races, capability fail-closed, pierwszą odpowiedź/replay, cache headers i Website `/auth/*` no-store boundary. Normalne unknown/ineligible/policy/delivery outcomes są nierozróżnialne; storage, corruption i inne wewnętrzne awarie pozostają generycznym 500 bez ujawniania szczegółów.
...
- [ ] SAFE-005 Dodać backup D1, R2 i każdego MailboxDO SQLite wraz z procedurą identity-preserving restore przed pierwszą migracją destrukcyjną. Decyzja operacyjna 2026-07-24: archiwum pozostaje Cloudflare-only w osobnym prywatnym R2 bucket z retention lock, szyfrowaniem, osobnymi least-privilege credentials i writerem bez prawa delete; przyjęte cele to RPO 24 h, RTO 4 h i retencja 35 dni. Maksymalnie 30-dniowe Cloudflare DO/D1 PITR nie wystarcza samodzielnie do celu retencji 35 dni, dlatego wymagane jest dodatkowe niemutowalne logiczne archiwum D1, każdego nazwanego MailboxDO i pełnego closure R2. Zadanie pozostaje otwarte i nieblokujące dla kolejnych niedestrukcyjnych prac foundation, ponieważ brak obecnie Cloudflare credentials, wdrożonego archiwum, zweryfikowanych maintenance controls, wspieranej procedury DO export/import oraz datowanego staging backup/identity-preserving restore drill. Live procedura musi udowodnić quiescence inboundu, outboundu i nowych Workflow, wspólny cutoff, restore R2 przed tym samym named DO i D1 na końcu oraz reconciliation alarmów, Workflow i nieokreślonych outboundów. Ukończony lokalny SAFE-015 nie spełnia SAFE-005. SAFE-005 nadal blokuje ukończenie etapu 1, produkcyjne uruchomienie firmowej poczty oraz każdą nową destrukcyjną migrację.
...
- [x] SAFE-007 Zachować transactional session oraz authorization recheck dla skończonego bieżącego inwentarza dziewięciu aplikacyjnych mutacji control plane z SAFE-006: owner bootstrap, rename, external recovery enroll/verify, recovery-code rotation, account-recovery consume, normal/recovery passkey finish oraz passkey revoke. Świeże mutacje uwierzytelnione recheckują w jednym D1 batchu exact token-bound session, database-time expiry, fail-closed requirement/capability shape, właściwą autoryzację i expected state przed zapisem mutacji, receipt i audytu; permission grant expiry także używa czasu wykonania D1. Public account-recovery consume zamiast nieistniejącej wcześniejszej sesji recheckuje exact flow, proof, identity version, user i recovery code względem czasu D1. Exact replay pozostaje actor/proof-bound receipt readback bez ponownego sprawdzania świeżego stanu. Ten skończony zakres nie obejmuje generycznego effect-auth protocol/session bookkeeping, ephemeral ceremony starts, development email/rate-limit storage, AI audit sink, migracji ani MailboxDO SQLite. Każda przyszła aplikacyjna mutacja control plane przyjmuje ten sam transactional session lub jawny proof-authorization, requirement, authorization, expected-state, atomic receipt i audit contract jako ogólny Definition of Done zamiast ponownie otwierać historyczny checkbox.
- [x] SAFE-008 Dodać bazowy limit rozmiaru inbound przed zapisem raw MIME do R2. Wspólny limit 10 MiB działa na granicy Cloudflare przed dekodowaniem adresów/routingiem i ponownie w use case przed ID/czasem/R2/Workflow, nie konsumuje streamu, a upload nadal sprawdza declared-vs-actual length przez `FixedLengthStream`. INB-009 pozostaje otwarte i musi zachować oraz przetestować guard na przyszłej ścieżce routingu.
- [x] SAFE-009 Dodać bazową regresję security invariants obecnego singletona przed usunięciem ograniczeń. Rozproszone testy właścicielskie bez sztucznej aggregate matrix potwierdzają owner allowlist, unrestricted i świeży step-up bootstrapu, transactional session/permission recheck oraz atomową odmowę drugiego bootstrapu z osobnym operation ID bez receipt, audytu ani częściowych zapisów. Bezpośrednie testy seamów są zaakceptowanym gate'em tej bazy: aktywne membership nadal wywołuje niezależną autoryzację, folder scope zawiera mailbox i odmawia reuse tego samego lokalnego folder ID między mailboxami, resolver mismatch jest defectem, caller mailbox ID pozostaje wyłącznie lookup hint, a routing używa SMTP envelope recipient. Testy realnych Layerów `MailboxIdentityDoLayer` i `MailboxDoHandlerLayer` wiążą identity z nazwą DO, odrzucają brak lub błędną nazwę oraz blokują forged payload przed wywołaniem store dla directory, mail data i resource lookup. Unikalny `app_mailbox_singleton_idx` jest sprawdzany drugą w pełni poprawną aktywną skrzynką. Jest to baseline przed usunięciem singletona, nie zgoda na jego usunięcie; przyszły model nadal wymaga testów wielu mailboxów i izolacji po migracji.
- [ ] SAFE-010 Uzupełnić recovery runbook o utratę właściciela, błędny grant i błędny routing. Kanoniczny dokument: [Owner, grant, and routing recovery](docs/runbooks/owner-grant-routing-recovery.md). Podstawa dokumentacyjna rozdziela bieżący singleton od przyszłego modelu organization, dopuszcza wyłącznie wspierane recovery i forward-fix oraz obecnie odmawia produkcyjnego break-glass. Zadanie pozostaje otwarte do wdrożenia i zweryfikowania maintenance controls, wspieranych grant/route repair operations oraz datowanego staging exercise z oczekiwanym i obserwowanym stanem, monitoringiem i evidence; SAFE-005 pozostaje osobnym obowiązkowym gate'em dla live backup i identity-preserving restore evidence.
- [ ] SAFE-011 Dodać CI gate dla migracji, typecheck, test, lint, format i build. Source contract jest wdrożony lokalnie: workflow obsługuje pull requesty, push do `main` i merge queue, używa przypiętych SHA oraz uruchamia osobno `bun run check`, `typecheck`, `test` i `build` na zamrożonych zależnościach. Zadanie pozostaje otwarte do uzyskania pozytywnego live GitHub run oraz skonfigurowania `ci` jako required status check w branch protection.
...
- [x] SAFE-013 Utrzymać recovery-safe identity policy dla enrollment i wszystkich login/recovery initiation paths; shared-routed address nigdy nie może być samodzielnym dowodem email-auth. Purpose-aware policy rozdziela external recovery od login initiation: oba cele odrzucają zarządzaną domenę, każdy zapisany mailbox route i aktywne recovery duplicates, a external recovery dodatkowo odrzuca aktywną login identity. Dekorowane usługi obejmują email OTP, magic link, publiczny i wewnętrzny email-verification start oraz password reset, zachowując rate limits, blokadę caller-provided secrets, active-password reset eligibility i fail-closed password sign-up/set. Migracja 1017 atomowo rozstrzyga wyścigi generic challenge z route: route unieważnia późniejszy consume niezużytego proof, a zużyty proof rezerwuje adres tylko do swojego expiry. Testy obejmują policy matrix, realny effect-auth composition, HTTP mapping i obie kolejności wyścigu. Recovery-only identity pozostaje osobnym modelem, public recovery nadal wymaga external link plus recovery code, invitation acceptance należy do INV-010, a pełne przyszłe shared-route mutations do ADDR-022. Każdy upgrade alpha contractu effect-auth wymaga przeglądu typów i metadata keys chronionych przez migrację 1017.
...
- [x] SAFE-015 Dodać lokalny, identity-preserving rehearsal restore pełnego stanu mailboxa do tego samego mailbox ID. `bun run test:mailbox-restore` tworzy niezależny capture-time snapshot `node:sqlite`, odtwarza wszystkie 15 autorytatywnych tabel wraz z historią migracji, schematem i FTS, osiem stanów outbound oraz dokładne raw MIME, inbound attachments i draft/outbound blobs przez produkcyjne key/metadata builders i readers; sprawdza closure, checksumy, zamknięty manifest, tamper/missing/foreign state, partial write/retry, no-clobber i sanitizowane evidence. Zakres pozostaje wyłącznie `mode=local-rehearsal`: Cloudflare nie był wykonany, object storage jest analogiem in-memory, alarm DO i stan Workflow nie są przechwytywane i wymagają reconciliation, self-digest nie dowodzi autentyczności, a hard-link publication jest lokalnym analogiem Linux. Kanoniczny runbook: [Mailbox backup and identity-preserving restore](docs/runbooks/mailbox-backup-restore.md). Live backup/restore pozostaje niewspierany i nieautoryzowany do zamknięcia SAFE-005.
...
- [x] ORG-001 Dodać `app_organization` ze statusem, wersją i niemutowalnym ID. Addytywna migracja 1018 i model domenowy definiują tenant root bez seeda, backfillu, membership, ownera, domeny ani singletona organizacji. Case-sensitive opaque ID ma zamrożoną gramatykę ASCII `[A-Za-z0-9_-]{1,128}`, jest niemutowalne, nieusuwalne i never-reused; wybór generatora pozostaje obowiązkiem ORG-006/ORG-012 przed pierwszym insertem. Rekord zaczyna jako `active`, version 1, z równymi safe-integer Unix millis `createdAt/updatedAt`; storage dopuszcza wyłącznie `active|suspended`, wymusza dokładny version increment i niemalejący czas, ale autoryzowane suspend/resume, receipt i audit nadal należą do ORG-013. Realny test upgrade 1017→1018 zachowuje istniejące dane i mailbox singleton, pozostawia nową tabelę pustą, dopuszcza wiele organizacji oraz blokuje invalid type/grammar, overflow, DELETE, REPLACE i insert-based upsert.
- [x] ORG-002 Dodać `app_organization_member` z aktywnym, zawieszonym i odwołanym lifecycle. Addytywna migracja 1019 przechowuje stabilne, never-reused membership epochs z osobnym opaque ID, FK `RESTRICT` do organizacji i globalnego użytkownika oraz częściową unikalnością jednej bieżącej pary organization/user. Storage dopuszcza wyłącznie `active -> suspended|revoked` i `suspended -> active|revoked`; revoke jest terminalny, nowa epoka po revoke wymaga nowego ID i czasu nie wcześniejszego od poprzedniego revoke. Exact version/timestamp/metadata constraints oraz blokady DELETE, REPLACE i insert-based upsert zachowują historię. Tabela pozostaje pusta: role, grants, existing-owner backfill, bootstrap, invitation acceptance i runtime membership gate należą do kolejnych zadań.
- [x] ORG-003 Zasiać zatwierdzone w DEC-017 organization i mailbox role mappings przez publiczny kontrakt kontekstu `authorization`. Kanoniczny kontrakt definiuje cztery osobno brandowane scope types, 25 permission definitions, siedem namespacowanych platform roles oraz dokładnie 58 immutable mappings: organization `8/7/1` i mailbox `16/13/9/4`; `send_identity.use` pozostaje direct-only. Addytywna migracja 1020 wykonuje fail-closed collision preflight, deterministyczny idempotentny seed i chroni canonical definitions/mappings przed generic update, delete, replace, upsert oraz dodatkowym mappingiem. Legacy `owner|manager|editor|viewer`, ich 40 mappingów, istniejące grants i bieżący owner bootstrap pozostają nietknięte do ACL-001; migracja nie tworzy organization grants, membership gate ani mailbox assignments. Kontekst `authorization` pozostaje właścicielem katalogu i scope encoding, a `organization` nie duplikuje tego kontraktu.
- [x] ORG-004 Dodać wersjonowaną IDNA domain canonicalization, wykonać preflight obecnej domeny i utworzyć `app_mail_domain` z globalnie wyłącznym canonical domain ownership. Deterministyczny profil V1 `mail-domain/ascii-alabel-input/uts46-nontransitional-std3/unicode-17/v1` przyjmuje wyłącznie wejście ASCII: zwykłe domeny i kanoniczne A-labels, normalizuje case do lowercase oraz odrzuca U-labels, Unicode width/dot variants i niekanoniczny Punycode. Każdy `xn--` jest sprawdzany przez przypięte `punycode@2.3.1`, NFC Unicode 17 z `@adraffy/ens-normalize@1.11.1`, `tr46@6.0.0` i recanonicalization, niezależnie od Unicode host runtime. Addytywna migracja 1021 pozostawia tabelę pustą, zachowuje lifecycle i historię claim epochs oraz wymusza globalną wyłączność bieżącej domeny. SQLite wymusza tylko możliwą do wyrażenia strukturę ASCII/DNS, w tym regułę `--` w pozycjach 3-4 wyłącznie dla `xn--`; pełny profil Punycode pozostaje granicą aplikacji. Nie istnieje jeszcze runtime write adapter, a każdy przyszły writer i reader musi kodować/dekodować przez `CanonicalMailDomain`/`MailDomainSchema`, ponieważ bezpośredni SQL może przechować strukturalny, ale niepoprawny A-label. Minima version/status to pending `>=1`, verified `>=2`, active `>=3`, suspended `>=4`, retired `>=2`; dozwolone cykle lifecycle mogą podnosić version ponad minimum. Deploy preflight jest pierwszym efektem stacka Alchemy i korzysta z tego samego `ConfigProvider`, w tym `--env-file`. Obsługa wejściowych Unicode U-labels jest jawnie odłożona do nowego, osobno wersjonowanego profilu.
- [x] ORG-005 Rozdzielić trzy role `MAILBOX_OWNER_EMAIL`. Wspólny, wymagany kontrakt konfiguracji zastępuje starą zmienną niepustą, unikalną JSON allowlistą `MAILBOX_BOOTSTRAP_OWNER_EMAIL_ALLOWLIST` o maksymalnie 32 kanonicznych identities oraz osobnym `MAILBOX_INITIAL_ADDRESS`; nie ma fallbacku do starej nazwy. Publiczny bootstrap command pozostaje bez adresu, Backend Worker wstrzykuje trusted initial address, a transakcja ponownie wymusza zgodność z konfiguracją. Addytywna migracja 1022 nie przepisuje parent receipts: backfilluje immutable V1 intent marker dla historycznego bootstrapu, zapisuje companion V2 dla nowych operacji i wiąże exact replay z initial address. Durable cutover sentinel zapobiega downgrade podczas reapply, a wąski trigger rolling-deploy tworzy V1 marker dla kończącego request starego writera i atomowo promuje go do V2 dla nowego protokołu. Resolver recovery-safe uzgadnia current `MailDomain`, retained legacy primary route i trusted pre-bootstrap domain; brak, wielość, malformed projection lub disagreement po bootstrapie są błędem storage, nigdy permission. Policy read i późniejszy auth-owned write nie tworzą cross-adapter transaction; ORG-009 musi serializować i rewalidować domain mutations.
- [x] ORG-006 Utworzyć legacy/default organization podczas migracji istniejącego wdrożenia. Addytywna migracja 1023 rezerwuje opaque migration identity `legacy_default_v1` i zapisuje dokładnie jeden immutable cutover outcome zamiast wybierać organizację przez count/order. `legacy-primary` wymaga jednego historycznego mailboxa `primary`, tworzy aktywną organizację version 1 z jego safe-integer `createdAt` i wiąże retained provenance; `fresh-empty` zapisuje brak obu rekordów bez syntetycznego inserta w migracji. Reapply nie interpretuje ponownie bieżących countów i akceptuje tylko dokładny sealed outcome. Storage blokuje częściowy fresh state, zmianę/delete/replace chronionego `primary` oraz obce lub wieloznaczne dane. Trusted bootstrap tworzy brakującą reserved organization pod tym samym authorization nonce i timestampem, atomowo przed pierwszym mailboxem; nie tworzy membership, organization grants, domain claim, mailbox organization binding ani tenant authority. Tymczasowy ORG-005 trigger V1 pozostaje częścią bieżącej promocji V2 i może zostać wycofany dopiero przez późniejszy atomowy storage protocol cutover.
- [x] ORG-007 Przypisać istniejący mailbox `primary` do legacy organization bez zmiany mailbox ID. Addytywna migracja 1024 dodaje retained, migration-only `app_mailbox_legacy_organization_assignment` oraz sealed cutover sentinel, nie wyprzedzając należącej do ORG-010 kolumny `app_mailbox.organization_id`. Dokładny V1 bridge wiąże `primary -> legacy_default_v1`, wspólny safe-integer creation timestamp i źródło `legacy-cutover|fresh-bootstrap`; FKs są `RESTRICT`, a update/delete/replace/upsert są zablokowane. Existing deployment jest backfillowany bez zmiany mailboxa ani innych danych; fresh-empty pozostaje pusty do pierwszego mailboxa, którego `AFTER INSERT` atomowo materializuje ancestry albo przerywa cały bootstrap. Reapply nie naprawia brakującego bridge, wymaga byte-exact własnego DDL i transitive trigger/index generation z ORG-006 oraz odrzuca column-era schema, aby nie wskrzesić tymczasowego triggera po ORG-010. Bridge nie jest organization membership, user `MailboxAssignment`, grantem ani authorization/discovery evidence.
- [x] ORG-008 Przypisać istniejącego właściciela jako aktywnego Organization Owner. Addytywna migracja 1025 nominuje usera wyłącznie z dokładnie jednego aktywnego, nieodwołanego, bezterminowego i metadata-null legacy `owner` grantu scoped do mailboxa `primary`; config allowlist, adres, creator, membership, receipt/audit, identity value, count/order i caller ID nigdy nie wybierają ani nie rozstrzygają kandydata. Exact ancestry, aktywny user i mailbox member, creator, oba role catalogs, brak globalnego legacy ownera oraz niesprzeczna historia tylko corroborują grant. Migracja tworzy active membership epoch `legacy_default_v1_owner_v1`, exact organization-scoped `organization.owner` grant, immutable assignment receipt i sealed cutover, zachowując legacy mailbox grant. Fresh `AFTER INSERT` audit trigger wiąże realny `mailbox.owner-bootstrap` audit, parent receipt i dokładnie jeden V1/V2 intent z tym samym atomowym bootstrapem; partial authority lub collision cofają całą batch. Nie powstaje syntetyczny historyczny audit, ponieważ obecna taksonomia nie koduje prawdziwie tej migracji. Reapply nie leczy brakujących artifacts ani nie odwraca późniejszego legal suspend/revoke. Membership sam nie jest permission, organization grant nie daje mailbox-content access, a pełny runtime membership gate pozostaje w ACL-003.
- [x] ORG-009 Utworzyć rekord domeny na podstawie istniejącego adresu i oznaczyć status migracji jawnie. Migracja 1026 rezerwuje fixed claim `legacy_default_v1_domain_v1` dla `legacy_default_v1`, zawsze jako `pending_verification` version 1, oraz osobno zapisuje sealed cutover i immutable receipt z exact route epoch, raw/normalized snapshots, pinned canonicalization profile i migration source. Route/config continuity ani claim nie są DNS ownership proof lub readiness. Populated legacy Punycode nie jest kanonikalizowany przez SQLite: wymagany, success-only cached Backend initialization Effect czyta jeden spójny D1 snapshot, dekoduje route/config/intents przez `canonicalizeMailDomainV1`, wymaga agreement i zapisuje claim/receipt przez atomową conditional batch przed obsługą HTTP lub inbound email. Fresh current writer stage'uje branded canonical intent przed auditem; rolling old writer przechodzi tylko dla jednoznacznego ordinary lowercase ASCII LDH, a A-label bez staging failuje atomowo. Reapply i startup nie leczą missing pair ani changed provenance, reserved lifecycle jest zamrożony do ORG-016, a pending claim pozostaje managed przez ORG-005 resolver bez rozszerzania effective domain set.
- [x] ORG-010 Dodać `organizationId` do mailbox registry oraz wymaganych indeksów. Forward-only migracja 1027 używa wyłącznie addytywnego `ALTER TABLE`, backfilluje canonical column przez retained ORG-007 bridge po `mailboxId` bez constant fallbacku i dodaje partial index `(organization_id, status, id) where deleted_at is null`. Fizyczne `notnull=0` jest ograniczeniem SQLite expand migration przy otwartym SAFE-005; committed-state contract wymaga non-null, istniejącego parenta i dokładnej equality column=bridge, chronionej triggerami i fail-closed readers. Jeden consolidated `AFTER INSERT` zastępuje ORG-007 trigger: tworzy bridge, uzupełnia null pominięty przez starego writera wyłącznie z bridge i sprawdza całość bez zależności od trigger order; current writer zapisuje trusted internal organization ID, public DTO pozostaje bez organization input. Ancestry i mailbox ID są niemutowalne, replace/upsert/delete są blokowane także przy wyłączonych FK, runtime initialization byte-validuje generation manifest, a receipt corruption zwraca storage zamiast not-found/conflict. Historyczne receipts/API pozostają wersyjnie niezmienione, singleton nadal obowiązuje, a ancestry sama nie jest authorization evidence. Migracja celowo nie jest ręcznie reaplikowalna, ponieważ SQLite nie ma `ADD COLUMN IF NOT EXISTS`; ledger zapewnia exactly-once, a ponowne uruchomienie failuje przed mutation.
- [x] ORG-011 Rozszerzyć user preferences o organization scope i zweryfikowany default mailbox. Forward-only migracja 1028 tworzy nową canonical `app_user_organization_preference` z PK `(organization_id,user_id)`, nullable defaultem, organization-scoped settings, exact safe time/version contract oraz FK do organization, usera i composite mailbox ancestry. Nie zmienia ani nie przebudowuje globalnie kluczowanej `app_user_preference`; legacy rows są backfillowane wyłącznie z exact sealed `legacy-primary` cutover, z zachowaniem JSON/timestamps/version i null defaultu, po czym legacy tabela jest migration-frozen do backup-gated contraction. Fresh-empty z legacy row, orphan user, malformed row lub cross-org/missing ancestry failują atomowo. `app_mailbox_organization_id_unique_idx` dostarcza composite parent key, a explicit triggery blokują FK-off cross-org writes, identity/time mutation, replace/upsert/delete i wymagają exact version increment. „Verified default” oznacza tylko canonical same-organization candidate; suspended/deleted default może pozostać dormant, a preference nie jest tenant selection, membership, discovery, assignment, grant ani access evidence. ORG-011 nie dodaje API, bootstrap write ani navigation consumption; MBX-006/007 muszą najpierw zbudować autoryzowany active candidate set i dopiero potem użyć preference jako ranking hint.
- [ ] ORG-012 Dodać serwis bootstrapu organizacji bez zaufania do organization ID przesłanego jako dowód dostępu. Phase A lokalnie ukończyła granicę aplikacyjną: publiczny `BootstrapOrganizationCommand` zachowuje wire `{displayName,operationId}` i jawnie odrzuca nazwane pola authority, `OrganizationBootstrap.make` przechwytuje trusted config i nowy port transakcji, HTTP tylko deleguje, a `MailboxAdministration` zachowuje wyłącznie rename/readOperation. Endpoint, actor w request context, batch D1, V1/V2 receipts, domain staging, `mailbox.owner-bootstrap`, triggery i pełny 15-row closure pozostają bez zmian. ORG-012 pozostaje otwarte, ale niezależna praca przechodzi na ORG-013: retirement compatibility triggerów wymaga jawnego dual-protocol expand/activate/drain/contract, a blokują go brak dowodu drenażu live writerów, potwierdzenia atomiczności dokładnej migracji Cloudflare D1, maintenance/quiescence z SAFE-010 oraz datowanego staging evidence; SAFE-005 pozostaje osobną otwartą bramą backup/identity-preserving restore. Docelowo zastąpić tymczasowe ORG-005/006/008/009/010 rolling bootstrap, ancestry-column i domain-staging triggery pełnym atomowym organization/mailbox/owner/domain protocol bez usuwania retained V1 intentów, ancestry, owner-assignment ani domain-claim receipts.
- [ ] ORG-013 Dodać lifecycle organization pierwszego wydania: `active <-> suspended`, z wersjonowaną macierzą dozwolonych operacji. Lokalna Phase A jest gotowa: migracja 1029 jest forward-only, wymaga exact live agreement z sealed 1028 predecessor i braku reserved artifacts, zapisuje sealed generation, durable `expanded` activation sentinel oraz addytywny organization-only audit stream zamiast przebudowy retained V1 audit table przy otwartym SAFE-005. Każda storage transition wymaga aktywowanego protokołu, pasującego immutable auditu z istniejącym aktorem i pustego fence set; receipt wiąże actor, exact intent/result, matrix V1 i `control-plane-sensitive` V1, a update/delete/replace, FK-off orphan i recursive-trigger-off replacement failują. Publiczne suspend/resume wymagają unrestricted token-bound session, pięciominutowego step-up, aktywnego exact organization membership i wyłącznie exact organization-scoped `organization.manage_settings`; global grant nie wystarcza. Operation ID zapewnia actor-bound readback, exact replay, conflict i unknown-commit recovery. Suspended organization zachowuje memberships, grants i child lifecycle, ale blokuje mailbox registry/navigation, inbound resolution i sender identity. Inbound commit oraz outbound provider dispatch używają osobnych holder-bound D1 fences: acquire jest możliwy tylko dla aktywnej organizacji, suspend wymaga pustego fence set, a każdy holder zwalnia wyłącznie własny fence; outbound alarm trzyma dodatkowy fence od chwili przed claimem do settlement, więc suspension nie ma claim race i nie zużywa retry budgetu. Protokół pozostaje celowo `expanded`, a suspend/resume failują do successor activation po wdrożeniu wszystkich writerów, drenażu starych Workflow/MailboxDO isolates, maintenance/quiescence i datowanym staging evidence; brak tych live dowodów blokuje pełne zamknięcie ORG-013 analogicznie do contract phase ORG-012. `deleting`, `deleted`, export i organization delete workflow pozostają odłożone do SAAS-006; niezależna praca przechodzi na ORG-014.
- [x] ORG-014 Dodać testy unikalności domeny, izolacji organizacji i atomowego bootstrapu. Udowodniono exact 15-row successful closure i pełny rollback przy awarii finalnego cleanupu guardu dopiero po materializacji całego closure. Realny bootstrap z foreign current domain ownerem zachowuje obcy tenant i cofa wszystkie reserved artifacts. Lifecycle transaction osobno odrzuca grant z organization A przy aktywnym membership w organization B, a storage triggery przy wyłączonych FK blokują cross-organization lifecycle audit, receipt i operation fence bez pomocniczego powodu odrzucenia. Suspended organization nie może dostarczyć sender identity mimo aktywnego mailboxa i adresu. Pełny gate objął 148 plików i 1726 testów oraz typecheck, lint/format, build i whitespace check; dwa niezależne review nie pozostawiły high/medium findings po wzmocnieniu testów.
- [ ] ORG-015 Udokumentować migrację i aktualny model w `README.md`.
- [ ] ORG-016 Dodać ownership challenge, proof expiry, re-verification i aktywację domeny. Przed dopuszczeniem nowej domeny atomowo zastąpić ORG-009 lifecycle freeze i dodać reciprocal serialization/revalidation między domain claim/handoff a recovery identities oraz live generic-email proofs; ORG-009 materializuje tylko już zarządzaną domenę i nie stanowi precedensu dla generic writera.
...
- [ ] MBX-006 Zastąpić `limit(1)` jawnym wybraniem mailboxa i deterministycznym fallbackiem. Najpierw wyznaczyć currently authorized active candidate set; ORG-011 preference nie może wybrać tenant scope ani ominąć assignment/permission.
- [ ] MBX-007 Zacząć używać `defaultMailboxId` per organizacja jako ranking hint po ponownej walidacji canonical ancestry, lifecycle, assignment i permission; stale/dormant default nie daje dostępu i nie wymusza implicit fallback do innej organizacji.
...
1. Foundation milestone: nie ma aktywnego lokalnego zadania SAFE; ukończone SAFE-001-004, SAFE-006-009 i SAFE-012-015, w tym lokalny rehearsal SAFE-015, pozostają baseline. Otwarte blokery SAFE-005, SAFE-010 i SAFE-011 nadal blokują zamknięcie etapu 1: SAFE-005 wymaga live Cloudflare archive/restore evidence, quiescence i reconciliation, SAFE-010 maintenance controls i staging exercise, a SAFE-011 live GitHub run oraz required status check.
2. Migration milestone: rozpocząć od ORG-001, następnie ORG-002; dalej organization bootstrap, domain ownership, role catalog, MailboxAssignment i migracja obecnego `primary`, nadal z aktywnym singletonem i bez destrukcyjnej migracji przed SAFE-005.
...
- 2026-07-25: Ukończono lokalnie wyłącznie ORG-012 Phase A: jeden `OrganizationBootstrap` jest aplikacyjną authority bootstrapu, publiczny command odrzuca nazwane pola authority, trusted config przechodzi przez nowy port, a `MailboxAdministration` nie zawiera bootstrapu. Wire/201, actor context, audit `mailbox.owner-bootstrap`, batch, replay/unknown commit, receipts, domain staging, migracje i triggery są niezmienione. ORG-012 nie jest zamknięte; compatibility retirement czeka na expand/activate/drain/contract, live writer-drain, Cloudflare D1 atomicity, SAFE-010 maintenance/quiescence i staging evidence. Niezależny pointer przesunięto na ORG-013, a SAFE-005 i SAFE-010 pozostają otwarte.
- 2026-07-25: Ukończono lokalną Phase A ORG-013: audytowany i idempotentny `active <-> suspended` lifecycle, fail-closed matrix V1, exact scoped authority oraz holder-bound D1 fences są wdrożone i testowane. Migracja 1029 pozostawia durable activation sentinel jako `expanded`, więc suspend/resume nie mogą uruchomić się podczas rolling deploymentu z możliwymi starymi Workflow/MailboxDO writerami. Pełne ORG-013 czeka na successor activation po live writer drain, maintenance/quiescence i staging evidence; niezależną pracę przeniesiono na ORG-014, a następną na ORG-015.
- 2026-07-25: Zamknięto ORG-014 defensywnym zestawem testów exact 15-row bootstrap closure, rollbacku przy finalnym guard cleanup, globalnej wyłączności domeny bez naruszenia foreign tenant, niezależnych organization membership/grant gates, cross-organization audit/receipt/fence storage binding oraz sender identity przy suspended organization. Testy eliminują pomocnicze powody odrzucenia, pełny gate obejmuje 148 plików i 1726 testów, a dwa niezależne review nie pozostawiły high/medium findings. Bieżącą pracę przeniesiono na ORG-015, a następną na ORG-016.

- 2026-07-24: Zamknięto lokalny SAFE-015 przez niezależny capture-time snapshot i restore pełnego MailboxDO/R2 closure do tego samego mailbox ID, z checksumami, manifestem, FTS, testami tamper/partial retry/no-clobber i sanitizowanym evidence. Runbook jawnie ogranicza wynik do `mode=local-rehearsal`; SAFE-005 pozostaje otwartą live Cloudflare bramą z wymaganą quiescence, logicznym 35-dniowym archiwum poza 30-dniowym PITR, reconciliation oraz datowanym staging drill. Bieżącą pracę przeniesiono na ORG-001, a następną na ORG-002, bez oznaczania etapu 1 jako ukończonego przy otwartych SAFE-005, SAFE-010 i SAFE-011.
- 2026-07-24: Zamknięto ORG-001 addytywnym, versioned tenant-root schema bez uruchamiania migracji legacy lub runtime mutation. ADR 0001 zamraża opaque ASCII, immutable i never-reused organization ID, storage lifecycle `active|suspended` oraz safe-integer time/version contract; bieżącą pracę przeniesiono na ORG-002, a następną na ORG-003.
- 2026-07-24: Zamknięto ORG-002 addytywnym organization membership lifecycle z retained epochs, terminalnym revoke, current-pair uniqueness i FK do organizacji oraz globalnego użytkownika. Nie włączono jeszcze ról, grantów, bootstrapu ani authorization gate; bieżącą pracę przeniesiono na ORG-003, a następną na ORG-004.
- 2026-07-24: Zamknięto ORG-003 publicznym authorization catalog i addytywnym seedem 25 permissions, siedmiu namespacowanych ról oraz 58 immutable mappings. Legacy grants/bootstrap pozostają świadomie bez zmian do ACL-001, organization owner grant do ORG-008, a membership gate do ACL-003; bieżącą pracę przeniesiono na ORG-004, a następną na ORG-005.
- 2026-07-24: Zamknięto ORG-004 deterministycznym profilem ASCII/A-label z przypiętym Unicode 17 NFC, neutralnym preflight jako pierwszym efektem stacka Alchemy i addytywną migracją globalnych retained domain claims. SQL chroni wyrażalną strukturę, natomiast pełna walidacja Punycode pozostaje obowiązkową granicą każdego przyszłego writera/readera; U-label input wymaga przyszłego wersjonowanego profilu. Bieżącą pracę przeniesiono na ORG-005, a następną na ORG-006.
- 2026-07-24: Wdrożono lokalny source contract SAFE-011 z przypiętymi SHA akcji, zamrożonym instalowaniem zależności i strukturalnym testem workflow. SAFE-011 pozostaje otwarte do pozytywnego live GitHub run oraz włączenia `ci` jako required status check w branch protection; bieżącą pracę przeniesiono na SAFE-013, a następną na SAFE-015.
- 2026-07-24: Zamknięto SAFE-013 przez purpose-aware recovery-safe identity policy, dekoratory wszystkich aktywnych generic email initiation services, fail-closed HTTP guards i migrację 1017 rozstrzygającą wyścig challenge-route. Bieżącą pracę przeniesiono na SAFE-015; upgrade effect-auth wymaga jawnego review vendor challenge metadata chronionego przez migrację.
- 2026-07-24: Zamknięto ORG-005 rozdzieleniem bootstrap owner allowlist, trusted initial mailbox address i managed-domain authority. Migracja 1022 zachowuje historyczne receipt V1 przez immutable intent markers, wiąże nowe bootstrapy companionem V2 i obsługuje atomowy rolling cutover bez możliwości fail-open lub reapply downgrade. Recovery-safe policy uzgadnia persisted `MailDomain`, legacy route i trusted pre-bootstrap claim, a storage disagreement kończy się sanitizowanym błędem wewnętrznym. Bieżącą pracę przeniesiono na ORG-006, a następną na ORG-007.
- 2026-07-24: Zamknięto ORG-006 migracją 1023 z deterministycznym `legacy_default_v1`, immutable outcome `legacy-primary|fresh-empty` i fail-closed ochroną provenance. Existing deployment dostaje tenant root z timestampem `primary`; fresh deployment zapisuje pusty cutover, a trusted bootstrap tworzy organizację atomowo przed mailboxem pod tym samym authorization guard. Nie nadano membership, grantów, domeny ani mailbox organization binding. Bieżącą pracę przeniesiono na ORG-007, a następną na ORG-008.
- 2026-07-24: Zamknięto ORG-007 retained ancestry bridge `primary -> legacy_default_v1` bez zmiany mailbox ID i bez wcześniejszego dodania canonical registry column. Migracja 1024 fail-closed backfilluje legacy state, materializuje fresh ancestry w transakcji inserta mailboxa, chroni exact generation i nie nadaje żadnego user authority. ORG-010 musi użyć bridge jako jedynego źródła column backfillu. Bieżącą pracę przeniesiono na ORG-008, a następną na ORG-009.
- 2026-07-24: Zamknięto ORG-008 fail-closed projekcją jednego exact legacy mailbox owner grantu do active organization membership i organization-scoped `organization.owner` grantu. Migracja 1025 zapisuje immutable authority receipt, obsługuje legacy no-history/audit-only/full receipt history oraz atomowy fresh rolling bootstrap bez wyboru ownera z configu lub incidental fields. Organization role nie rozszerza mailbox-content access, a membership gate nadal należy do ACL-003. Bieżącą pracę przeniesiono na ORG-009, a następną na ORG-010.
- 2026-07-24: Zamknięto ORG-009 fixed pending domain claimem i oddzielnym migration receipt zamiast fałszywego DNS verification status. Migracja 1026 instaluje fail-closed protocol, Backend reconciliation używa przypiętego TypeScript IDNA profile przed atomowym insertem legacy claimu, a fresh bootstrap stage'uje branded canonical intent; old writer ma wyłącznie bezpieczny ordinary-ASCII fallback. Claim nie rozszerza managed-domain policy i pozostaje lifecycle-frozen do ORG-016. Bieżącą pracę przeniesiono na ORG-010, a następną na ORG-011.
- 2026-07-24: Zamknięto ORG-010 addytywną canonical mailbox ancestry column, bridge-only backfillem i partial organization/status indexem bez rebuilda `app_mailbox`. Consolidated rolling trigger obsługuje starego oraz current writera bez trigger-order race, generation manifest i hot readers failują na null/disagreement, a historyczne receipts/API nie zostały reinterpretowane. Fizyczna nullable column pozostaje jawnie chronionym expand debt do czasu bezpiecznego rebuilda po SAFE-005. Bieżącą pracę przeniesiono na ORG-011, a następną na ORG-012.
- 2026-07-24: Zamknięto ORG-011 nową per-organization preference table zamiast niebezpiecznego rozszerzenia globalnego `user_id` PK. Migracja 1028 zachowuje legacy preferences, zamraża stare writes, wiąże non-null default przez composite canonical ancestry i pozostawia null/stale lifecycle values jako dormant ranking state bez authority. API, bootstrap i navigation są bez zmian do MBX-006/007. Bieżącą pracę przeniesiono na ORG-012, a następną na ORG-013.
- 2026-07-24: Ukończono lokalną Phase A ORG-012 przez wydzielenie jedynej application authority `OrganizationBootstrap`, przechwycenie trusted config poza HTTP i jawne odrzucanie caller authority fields bez zmiany wire/API, batch, receipts, audit lub storage triggers. Pełny ORG-012 pozostaje otwarty do dual-protocol rollout, live writer drain, Cloudflare D1 atomicity i maintenance/staging evidence; niezależną pracę przeniesiono na ORG-013, a następną na ORG-014.
