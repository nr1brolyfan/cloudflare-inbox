...
- retained control-plane migration ancestry is not user assignment or authority: `app_mailbox.organization_id` is canonical ancestry and must be non-null in valid committed state, exactly equal the permanently retained ORG-007 bridge by mailbox ID and creation provenance, and never be inferred from callers, users, grants, memberships, receipts, domains, routes, counts, or ordering. Its SQLite column is deliberately physically nullable during additive expansion while `SAFE-005` is open;
- organization-scoped user preference is optional ranking state only: it cannot select a tenant or establish membership, discovery, assignment, grant, permission, lifecycle eligibility, or access. Consumers must authenticate and authorize a current candidate set before consulting a default, and may use it only when the exact same-organization mailbox remains eligible. The retained legacy preference table is frozen and cannot be contracted while `SAFE-005` is open;
...
- organization lifecycle uses the closed `organization-operations` matrix V1. Suspend/resume require active membership, exact tenant-scoped permission, unrestricted session, fresh step-up, expected status/version, atomic audit and immutable receipt. Child lookup requires active state, while each asynchronous inbound commit and outbound provider dispatch holds an independently owned D1 fence that serializes its side effect against suspension; outbound alarms hold a fence across claim and settlement so suspended work consumes no queued attempt. The protocol remains `expanded` until a separately evidenced writer-drain successor activates it;
...
ORG-010 is the explicit forward-only exception to successful SQL reapplication. SQLite has no conditional additive `ALTER TABLE ADD COLUMN`, so migration 1027 requires an absent column and byte-exact predecessor, relies on the migration ledger for one application, and rejects manual re-entry before persistent mutation. Hot readers validate canonical row equality without repeating catalog scans; the existing success-only Backend initialization validates the immutable ORG-010 generation manifest against current byte-exact column, FK, index, bridge table/trigger, and owned trigger facts once per isolate and retries failures. Missing or tampered state is corruption for a future forward fix and is never healed by rerunning 1027. An ORG-010 trigger retained on the ORG-006 cutover table fences manual 1023 reapplication and transactionally restores 1023's tentative drops on failure.

The local migration harness wraps every migration file in `BEGIN IMMEDIATE` and proves preflight failures and manual predecessor/successor reapplications preserve complete schema and organization/mailbox/owner/domain state. It cannot inject an external failure between fixed statements after 1027's `ALTER TABLE` without modifying the migration under test; SQLite transaction semantics and the separate later-batch rollback test therefore remain the evidence for rollback after that point.
