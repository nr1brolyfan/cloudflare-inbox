...
- `app_mailbox.organization_id` is physically nullable only as an ORG-010 expansion constraint; a committed mailbox with null ancestry, bridge or sealed-cutover disagreement, missing parent, timestamp/provenance disagreement, or missing/tampered ORG-010 index, table, or trigger is corruption. Hot registry/navigation/routing reads fail closed on data disagreement, administration replay reports storage corruption for an existing receipt, and success-only Backend initialization checks the exact schema generation once per isolate before serving Backend traffic. Do not rerun migration 1027: it is ledger-controlled, forward-only, rejects manual reapplication before mutation, fences 1023 reapplication transactionally, and never heals artifacts or rows. Preserve the mailbox and bridge bytes for investigation and use only a reviewed successor forward fix after required backup controls exist.
