...
Migration 1025 retains the legacy `owner` mailbox grant and adds one exact `organization.owner` grant scoped to `legacy_default_v1`, with canonical membership provenance. The grant maps only to the eight organization permissions above and confers no mailbox, message, draft, attachment, folder, rule, or send authority. ORG-013 organization lifecycle commands locally enforce the future ACL-003 conjunction: active exact organization membership plus exact organization-scoped `organization.manage_settings`; a global grant cannot satisfy the operation. The immutable ORG-008 assignment receipt, not a fabricated `mailbox.owner-bootstrap` event, records the migration; fresh state links that receipt to the real existing mailbox bootstrap audit. Migration 1029 supplies the separate typed organization audit taxonomy required for suspend/resume.

Both suspend and resume require the versioned five-minute `control-plane-sensitive` step-up policy. Suspend is tenant-wide containment, while resume re-enables child access and delivery, so neither is available to an ordinary or recovery-restricted session. Exact operation receipts remain actor-bound for replay and unknown-commit readback without turning organization ID into access evidence.

Phase A does not create an `organization.bootstrap` audit action or change owner authorization semantics. The current `mailbox.owner-bootstrap` event remains truthful for the unchanged current mutation. Retiring compatibility writers or triggers requires the separate expand/activate/drain/contract protocol and operational evidence described by ADR 0001.
