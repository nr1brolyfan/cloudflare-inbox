...
SAFE-002 and SAFE-006 are complete. Public recovery start uses exact ten-minute IP/email rate-limit rules, a 500 ms response floor, and detached Worker `waitUntil` delivery. Normal unknown, disabled, no-code, recovery-safe policy-denied, and delivery-failure paths are intentionally indistinguishable; dependency, storage, and data-corruption failures remain sanitized generic 500 responses rather than being misclassified as normal ineligibility. Every browser-facing `/auth/*` response is private and no-store at the Website proxy boundary. SAFE-003 remains open for the mailbox-wide versioned session-requirements matrix, and SAFE-013 remains open for the remaining generic login/recovery initiation integration.
