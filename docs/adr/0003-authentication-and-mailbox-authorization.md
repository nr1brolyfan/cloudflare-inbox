...
SAFE-006 is complete for the finite current mutation inventory. SAFE-002 remains open only for its remaining direct denial/conflict and HTTP evidence around detached `waitUntil` delivery, rate limiting, enumeration-safe responses, and `no-store`. The implemented two-proof and restricted-remediation design is accepted; the open task does not reverse that decision.
