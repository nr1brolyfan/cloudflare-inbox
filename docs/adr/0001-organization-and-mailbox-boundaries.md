...
- `app_mailbox` intentionally has no organization column in this generation. ORG-010 must backfill its canonical column solely by joining the retained bridge on `mailbox_id`, must retain the bridge afterward, and must atomically replace the temporary materialization trigger with the column-era protocol. It may not infer ancestry from constants, row counts, ordering, users, grants, memberships, domains, or navigation state.
