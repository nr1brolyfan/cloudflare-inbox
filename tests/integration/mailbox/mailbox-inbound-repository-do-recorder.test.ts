...
import { InboundProcessingRecorderDoLayer } from "#/modules/mailbox/adapters/durable-object/MailboxInboundRepositoryDo";
import { RecordInboundProcessingV1 } from "#/modules/mailbox/domain/MailboxInbound";
import { InboundProcessingRecorder } from "#/modules/mailbox/ports/MailboxInboundRepository";
...
const runRecord = (executeMailData: MailboxDoStub["executeMailData"]) =>
  Effect.runPromise(
    InboundProcessingRecorder.pipe(
      ...
      Effect.provide(
        InboundProcessingRecorderDoLayer.pipe(
          Layer.provide(
            Layer.succeed(
              MailboxDoNamespace,
